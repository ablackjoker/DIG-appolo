...
bool MacroSolver::applyNSEG13BoundaryFluxes()
{
    ...
    vector<int> topWallFaces;
    wallFaces.reserve(this->wallVis.size());
    topWallFaces.reserve(this->topVis.size());
    ...
    for (int faceIdx : this->topVis)
    {
        if (faceIdx < 0 || faceIdx >= this->Nface) continue;
        const BoundaryCondition& bc = this->boundaryTable.byTag(this->edges[faceIdx].faceTag);
        if (bc.nsRole == BoundaryRole::TopWall)
            topWallFaces.push_back(faceIdx);
    }
    this->Flux_NSEG13_bcWallWithT(this->mess.Twall_ref, wallFaces);
    this->Flux_NSEG13_bcWallwithVelocity(
        this->mess.Twall_ref, this->mess.u_wall * sqrt(2.0), topWallFaces);
    return true;
}
...
bool MacroSolver::initialBC()
{
    ...
    double rho = 1, ux = this->mess.u_wall*sqrt(2.0), uy = 0, uz = 0,t = this->mess.T_in/this->mess.T_ref;
    for (int i = 0, faceIdx; i < this->topVis.size(); i++){
        faceIdx  = this->topVis[i];
        qfr[faceIdx][0] = rho;
        qfr[faceIdx][1] = ux;
        qfr[faceIdx][2] = uy;
        qfr[faceIdx][3] = uz;
...
bool MacroSolver::SLAU2(bool gsisTag)
{
    int inface = this->iNface, nface = this->Nface, enface = this->eNface;
    int ncell = this->Ncell, incell = this->iNcell, var = this->var;
    edge *edges = this->edges;
    double **rhs = this->RHS, flux;
    double m = 1.0, pt = 1.0, mp, mn;
    double *fl = new double[var], *fr = new double[var];
    double *ul = new double[6], *ur = new double[6];
    double *pe = new double[var];
    for(int j = 0; j < var; j++){
        pe[j] = 0.0;
    }
    ...
    for(int i = 0; i < nface; i++){
        ...
        pe[0] = pe[4] = pe[5] = 0.0;
        pe[1] = edges[i].edgeNormal[0];
        pe[2] = edges[i].edgeNormal[1];
        pe[3] = edges[i].edgeNormal[2];
        for(int j = 0; j < var; j++){
            fl[j] = Qf[l][j];
            if(r < ncell){
        ...
        this->slau2Function(fl, fr, ul, ur, m, pt, edges[i].edgeNormal);
        mp = 0.5 * (m + fabs(m));
        mn = 0.5 * (m - fabs(m));
        this->clacSlauPsi(qfl[i], fl);
        this->clacSlauPsi(qfr[i], fr);
        for(int j = 0; j < var; j++){
            if(i < inface || i >= enface){
                flux = (mp * fl[j] + mn * fr[j] + pt * pe[j]) * edges[i].length;
            }else{
                flux = (mp * fl[j] + mn * fr[j] + 0.5 * pt * pe[j]) * edges[i].length;
            }
            if(l < incell){
                rhs[l][j] -= flux;
            }else if(l < ncell){
    ...
    delete[] fl; delete[] fr; delete[] ul; delete[] ur; delete[] pe;
    return true;
}

bool MacroSolver::slau2Function(double *fl, double *fr, double *ul, double *ur, double &m, double &pt, const double *enormal)
{
    double c, c2, mk, ka, unp, unn, un, pl, pr, plp, prn, g;
    this->slau2Function2(fl, ul, enormal);
    this->slau2Function2(fr, ur, enormal);
    c = 0.5 * (ul[3] + ur[3]);
    c2 = sqrt(0.5 * (ul[4] + ur[4]));
    mk = min(1.0, c2 / c);
    ka = pow(1.0 - mk, 2);
    un = (fl[0] * fabs(ul[5]) + fr[0] * fabs(ur[5])) / (fl[0] + fr[0]);
    ul[0] = ul[5] / c;
    ur[0] = ur[5] / c;
    g = -max(min(ul[0], 0.0), -1.0) * min(max(ur[0], 0.0), 1.0);
    unp = (1.0 - g) * un + g * fabs(ul[5]);
    unn = (1.0 - g) * un + g * fabs(ur[5]);
    pl = fl[0] * fl[4];
    pr = fr[0] * fr[4];
    m = 0.5 * (fl[0] * (ul[5] + unp) + fr[0] * (ur[5] - unn) - ka / c * (pr - pl));
    plp = this->psLeft(ul[0]);
    prn = this->psRight(ur[0]);
    pt = 0.5 * (pl + pr) + 0.5 * (plp - prn) * (pl - pr) + c2 * (plp + prn - 1.0) * sqrt(fl[0] * fr[0]) * c;
    fl[0] = 1.0;
    fr[0] = 1.0;
    fl[4] = ul[1];
    fr[4] = ur[1];
...
bool MacroSolver::slau2Function2(const double *f, double *ut, const double *enormal)
{
    double rho = f[0], u = f[1], v = f[2], w = f[3], ttra = f[4], trot = f[5];
    double ma, ht, hr, c, c2, un, tot;
    double gamma = mess.gamma, dr = mess.dr, cv = mess.cv;
    un = u * enormal[0] + v * enormal[1] + w * enormal[2];
    c = sqrt(gamma * ttra);
    ma = un / c;
    c2 = u * u + v * v + w * w;
    tot = (3.0 * ttra + dr * trot) / (3.0 + dr);
    ht = gamma * cv * rho * tot + 0.5 * rho * c2;
    hr = 0.5 * dr * rho * trot;
    ut[0] = ma;
    ut[1] = ht / rho;
    ut[2] = hr / rho;
...
void MacroSolver::FourierUpdateDensity()
{
    if (!active()) return;
    double den = 0.0;
    for (int i = 0; i < this->iNcell; i++){
        den += this->Qf[i][0] * this->cells[i].area;
    }
    MPI_Allreduce(MPI_IN_PLACE, &den, 1, MPI_DOUBLE, MPI_SUM, this->myGroup);
    if (den > 0.0){
        const double scale = this->mess.Area / den;
        for (int i = 0; i < this->iNcell; i++){
            this->Qf[i][0] *= scale;
        }
    }
...
bool MacroSolver::Flux_NSEG13_bcWallwithVelocity(double T, double u, vector<int> wallVis)
{
    edge *edges = this->edges;
    int l = 0, r= 0, v = 0;
    double Frho, Frhou, Frhov, Frhow, FrhoE, FrhoEr;
    ...
    double lam_tt, length, *norm = NULL;
    int dim = DIM;
    double *gm = new double[var * dim];// *gr = new double[var*dim];
    double *ds = new double[VAR2];
    double *wrw = new double[VAR], *sqrw = new double[VAR2], *sqlw = new double[VAR2];
    double *A = new double[8], localuw;
    bool left = true, right = false;
    double rho = 1, ux = u, uy = 0, uz = 0, t = T;
    double x1, y1, z1, x2, y2, z2, x3, y3, z3;
    for(int i=0, index; i<wallVis.size(); i++){
        v = wallVis[i];
        index = (v - eNface) * 2 * DIM;
        ...
        norm = edges[v].edgeNormal;
        this->calcG13Sigma(l, gm, ds, sqlw);
        // this->calcFlux_IFV2(qfl[v], sqlw, norm, IFvl, A, left, index);
        this->calcFlux_IFV2(Qf[l], sqlw, norm, IFvl, A, left, index);
        wrw[1] = ux; wrw[2] = uy; wrw[3] = uz; wrw[4] = t; wrw[5] = t;
        sqrw[0] = 0; sqrw[1] = 0; sqrw[2] = 0; sqrw[3] = 0; sqrw[4] = 0; sqrw[5] = 0; sqrw[6] = 0; sqrw[7] = 0;
        sqrw[8] = 0; sqrw[9] = 0; sqrw[10] = 0; sqrw[11] = 0;
