...
void meshImport::setMeshMessage()
{
    ...
    double kB = 1.3806e-23,T_ref = 273.15,n_ref = 2.685e25,p_mass = 4.65e-26,p_mass_r = 0.5*p_mass;
    double d_ref = 4.17e-10,alpha = 1.0, Twall_ref = 1.0,eta = omega - 0.5;
    double T_in = 273.15;
    double p_ref = n_ref*kB*T_in;
    double v_rms = sqrt(2*kB*T_ref/p_mass);
    double v_in = sqrt(2*kB*T_in/p_mass);
    const double d_ref2 = d_ref * d_ref;
    double miu_ref = 5.0 * (alpha+1.0) * (alpha+2.0) * sqrt(p_mass*kB*T_in/M_PI)
